...
<body>
	...
	<div class="container qnaCT">
		...
		<form action="${conPath }/Item/itemBoardModify.do" name="frm" enctype="multipart/form-data" method="post" onsubmit="return false;">
			<input type="hidden" value="${pageNum }" name="pageNum" /> 
			<input type="hidden" value="" name="colorList" id="colorList" /> 
			<input type="hidden" value="${itbList.ibnum }" name="ibnum" />
			
			<table>
				<tr>
					<th>제목</th>
					<td><select name="ibcategory">
							<c:if test="${itbList.ibcategory eq '귀걸이'}">
								<option selected="selected">
							</c:if>
							<c:if test="${itbList.ibcategory ne '귀걸이' }">
								<option>
							</c:if> 귀걸이
							</option>
							<!-- 나중에 카테고리 테이블 만들어서 불러오기 -->
							<c:if test="${itbList.ibcategory eq '목걸이'}">
								<option selected="selected">
							</c:if>
							<c:if test="${itbList.ibcategory ne '목걸이' }">
								<option>
							</c:if>목걸이
							</option>
					</select> 
					
					<input type="text" required="required" name="ibtitle" value="${itbList.ibtitle }" placeholder="주의! 제목은 등록시 부품 아이디로 저장됩니다." />
				...
				<tr>
					<th>내용</th>
					<td><textarea rows="30" cols="151" name="ibcontent" required="required">${itbList.ibcontent }</textarea> </td>
				</tr>
				...
				<%-- <tr>
					...
					<input type="button" onclick="location.href='${conPath}/Item/itemBoardList.do'"
						value="목록" /></td>
				</tr> --%>
			...
			<div id="info_buttons">
		        <button onclick="save()" class="qnart_btn focus" value="저장">저장</button>
		        <button onclick="location.href='${conPath}/Item/itemBoardList.do'" class="qnart_btn focus" value="목록">목록</button>
	        </div>
