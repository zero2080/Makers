...
<div class="container itemCT">

	<form action="${conPath }/Item/itemBoardWrite.do" name="frm" enctype="multipart/form-data" method="post" onsubmit="return false;">
	<input type="hidden" value="test" name="mid" />
	<input type="hidden" value="" name="colorList" id="colorList" />
	
	<table class="table itemWT">
		<tr>
			<th>제목</th>
			<td><select name="ibcategory">
					<option>귀걸이</option><!-- 나중에 카테고리 테이블 만들어서 불러오기 -->
					<option>목걸이</option>
				</select>
				<input type="text" required="required" name="ibtitle" size="149" placeholder="주의! 제목은 등록시 부품 아이디로 저장됩니다."/></td>
		</tr>
		<tr>
			<th>내용</th>
		...
		<tr >
			<td>
				<div id = "container"  >
					...
					<input type="button" onclick="insert();" value="색상 저장"/>
					<br>
				</div>
			</td>
		</tr>
		...
		<div id="info_buttons">
	        <button onclick="save()" class="itemwrt_btn focus">저장</button>
	        <button onclick="location.href='${conPath}/Item/itemBoardList.do'" class="itemwrt_btn focus">목록</button>
        </div>
