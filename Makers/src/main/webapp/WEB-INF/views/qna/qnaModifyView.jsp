...
<jsp:include page="../main/header.jsp"/>
	...
	<form action="${conPath }/QnA/qnaModify.do" enctype="multipart/form-data" method="post">
	...
	<c:if test="${not empty admin }">
		<c:set var="info" value="${admin.aid }"></c:set>
	</c:if>
	
		<table class="table qnaWT">
			<tr>
				<th>제목</th>
				<td><input type="text" required="required" name="qtitle" value="${list.qtitle }"/></td>
			</tr>
			<tr>
				<th>내용</th>
				<td><textarea rows="30" cols="151" name="qcontent" required="required">${list.qcontent }</textarea> </td>
			</tr>
			<tr>
				<th>파일</th>
		...
		<div id="info_buttons">
	        <button onclick="history.go(-1)" class="qnart_btn focus">이전</button>
	        <button type="submit" class="qnart_btn focus">수정</button>
        </div>
